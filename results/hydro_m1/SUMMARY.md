...
### 5.2 `results/parent_pristine/` 의 기록된 부호와 어긋난다 — 원인 확인 중

`results/parent_pristine/PN_out_chirality_polarplane.txt` 는
`(+0.14649, +0.07085, 0)` 에서 **+1** 로 적혀 있다. 그 런의 반지름은
0.0100 (= 삼중항을 감싸는 크기)인데, 같은 반지름으로 다시 재면 **−1** 이고
독립 구면 flux 도 −1 이다.

차이나는 설정은 `Nk2` 뿐이다 (기록 200, 이번 801). CLAUDE.md 가 quasi-2D 콘에는
`Nk2 ≥ 801` 이 필요하다고 이미 적어 둔 것과 맞물리므로 수렴 부족이 유력하지만,
추측으로 두지 않고 **기록된 `pn_weylchirality.in` 을 글자 그대로 한 번,
`Nk2` 만 801 로 바꿔 한 번** 돌려 원인을 특정하는 중이다.
결과가 나오면 이 절과 `results/parent_pristine/SUMMARY.md` 를 함께 고친다.

> 크기(|χ|=1)와 "알짜 0 인 4점 궤도" 라는 구조는 어느 쪽이든 바뀌지 않는다.
> 문제는 `(+,+)` 자리의 부호 하나다.
