...
**끊는 방법**: NaN 만 끼우면 선이 끊기긴 하지만 폭이 0 이라 *단차*처럼 보인다.
그래서 그 뒤의 x 를 전체 폭의 2.2 % 만큼 통째로 밀어 **실제로 벌어진 흰 틈**을
만들고, 그 자리에 NaN 을 넣는다. 데이터 값은 하나도 건드리지 않는다 —
경로 길이 축이 원래 임의이므로 정당하다. Γ 눈금은 틈 한가운데, 세로선은 틈의
양쪽 경계에 긋는다. 끊는 곳은 **안쪽 Γ 한 군데뿐**이다.
